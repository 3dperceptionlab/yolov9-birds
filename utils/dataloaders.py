...
class LoadStreams:
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True, transforms=None, vid_stride=1):
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            st = f'{i + 1}/{n}: {s}... '
            is_rtsp = urlparse(s).scheme == 'rtsp'
    ...
    def update_picam(self, i, picam, stream):
        # Read stream `i` frames in daemon thread
        n, f = 0, self.frames[i]  # frame number, frame array
        while picam.started and n < f:
            n += 1
            try:
                ...
                if n % self.vid_stride == 0:
                    if frame is not None:
                        ...
                        LOGGER.warning('WARNING: Video stream unresponsive, please check your camera connection.')
                        self.imgs[i] = np.zeros_like(self.imgs[i])
                        picam.stop()
                        picam.start()
            except Exception as e:
                LOGGER.warning(f'WARNING: Video stream error: {e}')
                self.imgs[i] = np.zeros_like(self.imgs[i])
                try:
                    picam.stop()
                    picam.start()
                except Exception as restart_error:
                    LOGGER.error(f'ERROR: restart camera stream: {restart_error}')
